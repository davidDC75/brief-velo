...
let url = 'http://90.110.218.245:5003'
let etapes = null
...
function carte(etapes) {
    ...
    for (let etape of etapes) {
        console.log(etape);
        console.log(url+etape.attributes.gpx.data[0].attributes.url);
        isFirst = (i == 0)
        isLast = (i == etapes.length - 1)
        etape.gpx = new L.GPX(url + etape.attributes.gpx.data[0].attributes.url, {
            async: true,
            marker_options: {
                shadowUrl: '',
                shadowSize: [0,0],
